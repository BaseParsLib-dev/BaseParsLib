...
def test_split_on_chunks_by_chunk_len() -> None:
...
def test_split_on_chunks_by_count_chunks() -> None:
    ...
    assert result == [
        ...
        [],
        [],
    ], "Ошибка при делении на чанки, когда количество чанков больше длины массива"


# Тесты для split_dict_on_chunks_by_chunk_len
def test_split_dict_on_chunks_by_chunk_len() -> None:
    # Тестирование деления словаря на 2 чанка
    ...
    # Тестирование деления словаря на 3 чанка
    result = list(split_dict_on_chunks_by_chunk_len({"a": 1, "b": 2, "c": 3, "d": 4}, 3))
    assert result == [
        {"a": 1, "b": 2, "c": 3},
        {"d": 4},
    ], "Ошибка при делении словаря на чанки по длине"
    ...
    assert result == [
        {"a": 1, "b": 2}
    ], "Ошибка при делении на чанки, когда длина чанка больше количества элементов в словаре"
